...
module.exports = (req, res, next) => {
  ...
  const modernRequest = req as ReposAppRequest;
  const gitHubId = modernRequest.individualContext.getGitHubIdentity().id;

  getReposAndOptionalTeamPermissions(orgName, operations, gitHubId, teamsType, team2, specificTeamId, (error, reposData, ageInformation, repoPermissions, userRepos, specificRepositories) => {
    if (error) {
      return next(error);
    }
