...
const definitions = require('./teamDefinitions.json');
...
function processTeams(context) {
  return getTeams(context)
    .then(iterateTeams);
}

function identityMaintainersWithoutLinks(teamContext, maintainers) {
...
function identifyTeamMaintainers(teamContext, team) {
  const cacheOptions = {
    backgroundRefresh: false, // immediate
    maxAgeSeconds: 60 * 60 * 5, // 5 hours
  };
  const deferred = Q.defer();
  team.getMaintainers(cacheOptions, (getMaintainersError, maintainers) => {
    if (getMaintainersError) {
      return deferred.reject(getMaintainersError);
    }
    teamContext.teamMaintainers = maintainers;
    return deferred.resolve(maintainers);
  });
  return deferred.promise;
}

function iterateTeams(context) {
  let teams = context.entities.teams;
  context.processing.teams = {
    remaining: teams.length,
  };
  const limit = qlimit(1);
  const process = processTeam.bind(null, context);
  return Q.allSettled(teams.map(limit(process))).then(() => {
...
function getTeamDetails(teamContext) {
  const deferred = Q.defer();
  ...
  team.getDetails(cacheOptions, error => {
    if (error) {
      return deferred.reject(error);
    }
    return deferred.resolve(teamContext);
  });

  return deferred.promise;
}

function getIndividualUserLink(maintainer) {
  const deferred = Q.defer();
  maintainer.resolveDirectLink(linkError => {
    return linkError ? deferred.reject(linkError) : deferred.resolve();
  });
  return deferred.promise;
}

function gatherLinkData(teamContext, maintainers) {
  const limit = qlimit(2);
  return Q.allSettled(maintainers.map(limit(maintainer => {
    return getIndividualUserLink(maintainer);
  }))).then(() => {
    return Q(maintainers);
  }).catch(error => {
    throw error;
  });
}

function processTeam(context, team) {
  ...
  const resolveLinks = gatherLinkData.bind(null, teamContext);
  const getTeamMaintainers = identifyTeamMaintainers.bind(null, teamContext, team);
  const getUnlinkedMaintainers = identityMaintainersWithoutLinks.bind(null, teamContext);
  return getTeamDetails(teamContext)
    .then(getTeamMaintainers)
    .then(resolveLinks)
    .then(getUnlinkedMaintainers)
    .then(() => {
      ...
      basicTeam.maintainers = teamContext.maintainers && teamContext.maintainers.length ? teamContext.maintainers.length.toLowerCase() : 'None';
...
// ----------------------------------------------------------------------------
//
// End unique teams.js code
//
// Below this line, the code is duplicated from the existing job code. It should eventually be refactored
// to use a shared module. No need to review the following...
// ----------------------------------------------------------------------------
...
function addEntityToIssueType(context, teamContext, type, entity, optionalAction1, optionalAction2) {
...
function flattenTeamsMap(teamsMap, operations) {
  const asArray = Array.from(teamsMap.values());
  const values = [];
...
function getTeams(context) {
  ...
  operations.getTeams(null, (error, teams) => {
    if (error) {
      return deferred.reject(error);
    }
...
function transformReasonsToArray(userEntry, teamName, orgName) {
  ...
  if (userEntry.reasons.directCollaborator) {
    ...
    for (let i = 0; i < userEntry.reasons.memberships.length; i++) {
      ...
      reasons.push(`Maintainer of the ${team.name} team in the ${orgName} GitHub organization`);
    }
  }

  if (!reasons.length) {
    reasons.push(`Unknown reason related to the ${teamName} team in the ${orgName} GitHub organization`);
  }
  return reasons;
}
