...
export function getPrioritizedOrganizationsList(operations: Operations, options?: PrioritizedOptions) {
  ...
  const absoluteFirst = organizations.filter(o => o.getDynamicSettings().hasFeature('official-company-organization'));
  const primary = organizations.filter(o => o.getDynamicSettings().getProperty('priority') === 'primary');
  const secondary = organizations.filter(o => o.getDynamicSettings().getProperty('priority') === 'secondary');
