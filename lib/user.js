...
OpenSourceUser.prototype.corporateProfileUrl = function () {
  ...
  var corporateSettings = this.oss.setting('corporate');
...
OpenSourceUser.prototype.unlinkAndDrop = function (callback) {
  const self = this;
  const oss = self.oss;
  const operations = oss.operations;
  let eventData = null;
  self.getLink((getLinkError, linkObject) => {
    // We ignore the get link error: should must be able to process drops without a link
    if (linkObject) {
      const aadIdentity = {
      ...
      // Fallback to providing any available GitHub information
      eventData = {
        github: {
          id: oss.id.github,
          login: oss.usernames.github,
        },
      };
    }
  });
  self.getActiveOrganizationMemberships((getOrganizationsError, currentOrganizationMemberships) => {
    ...
    async.each(currentOrganizationMemberships, function (org, cb) {
      ...
      dc.removeLink(oss.id.github, (removalError) => {
        oss.insights.trackEvent('UserUnlink');
        oss.invalidateLinkCache(() => {
          if (operations) {
            operations.fireUnlinkEvent(eventData);
          }
          // TODO: Notify via link events that an unlink operation has happened
          callback(removalError);
        });
      });
    });
  });
...
OpenSourceUser.prototype.updateLink = function (updatedLink, callback) {
  ...
  dc.updateLink(self.id, updatedLink, (updateError) => {
    ...
    self.oss.invalidateLinkCache('aad', updatedLink.aadoid || 'no-aad-oid', () => {
