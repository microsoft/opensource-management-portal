...
import { getSafeCosmosResourceKey } from '../transitional.js';
import type { ISimplifiedCosmosHelper } from '../cosmosHelper.js';
...
export interface ICampaignHelperSimplifiedDirectDataStore {
  getDirectDataStore(): ISimplifiedCosmosHelper;
}
...
export class StatefulCampaignProvider implements ICampaignHelper, ICampaignHelperSimplifiedDirectDataStore {
  constructor(private cosmos: ISimplifiedCosmosHelper) {}
  ...
  async getState(
    ...
    try {
      const groupData = await this.cosmos.getObject<CampaignUserStateRaw>(corporateId, groupKey);
    ...
    try {
      if (campaignId) {
        const data = await this.cosmos.getObject<CampaignUserStateRaw>(corporateId, key);
        if (data && data.sent) {
  ...
  async optOut(corporateId: string, campaignGroupId: string): Promise<void> {
    ...
    await this.cosmos.setObject(value);
  }
  ...
  async setAny<T>(corporateId: string, campaignGroupId: string, campaignId: string, data: T) {
    ...
    await this.cosmos.setObject(value as any);
  }

  async getAny<T>(documentId: string, partitionKey = ''): Promise<CampaignStateWithData<T>> {
    try {
      const document = await this.cosmos.getObject(partitionKey, documentId);
  ...
  async deleteOops(corporateId: string, campaignGroupId: string): Promise<void> {
    ...
    try {
      await this.cosmos.delete(corporateId, id);
    } catch (oopsError) {
