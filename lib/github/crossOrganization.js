...
function createMethods(libraryContext, collectionsClient) {
  function generalizedCollectionMethod(token, apiName, method, options, cacheOptions, callback) {
    console.log(`generalized collection method ${apiName}`);
    ...
    function wrapCall(icb) {
      return (xe, xr) => {
        console.log('before wrap callback');
        return icb(xe, xr);
      };
    };
    const tempCallback = callback;
    callback = (xe, xr) => {
      console.log(`generalized collection method cb ${apiName}`);
      if (apiName === 'teams') {
        const teamHalt = true;
      }
      tempCallback(xe, xr);
    };
    return core.execute(apiContext, wrapCall(core.createCallbackFromEntityData(callback, `XORG ${apiName}`)));
  }

  function getCrossOrganizationMethod(orgsAndTokens, apiName, methodName, options, cacheOptions, callback) {
    // temp
    console.log(`getCrossOrganizationMethod ${apiName} ${methodName}`);
      ...
      async.eachOfLimit(orgsAndTokens, 1, (token, orgName, next) => {
        ...
        method(token, localOptions, localCacheOptions, (orgError, orgValues) => {
          ...
          if (orgValues && orgValues.data) {
            console.log(`${apiName} ${methodName} result has data... use it?`);
            values.orgs[orgName] = orgValues.data;
            return next();
          }
          // TODO before merge:
          // not sure whether we SHOULD expect data or not, since the result
          // can come a few ways to here...
          console.log(`${apiName} ${methodName} result has no data property but inherantly has the values, hopefully`);
        ...
        console.log('-- returning a .data object --');
        //console.log('-- returning a projected, flatter object without a .data --');
        const dataObject = {
          data: values,
          meta: values.meta,
        };
    ...
    callback = (xe, xr, xmisc) => {
      console.log('real callback here');
      realCallback(xe, xr, xmisc);
    };
    //console.log(`returning generalizedCollectionMethod with a flattening callback for ${apiName}`);
    return generalizedCollectionMethod(orgsAndTokens, apiName, crossOrgMethod, options, cacheOptions, callback); // core.createCallbackWithFlattenedData(callback));
  }
    ...
    return () => {
      ...
      outerFunction(orgsAndTokens, {}, cacheOptions, (outerError, data) => {
        ...
        if (!outerError && data && !data.data) {
          const msg = 'crossOrganizationCollection inner outerFunction returned an entity but no entity.data property was present';
          console.log(msg);
          const halt = true;
          outerError = new Error(msg);
        ...
        async.eachLimit(Object.getOwnPropertyNames(entitiesByOrg.orgs), 1, (orgName, nextOrg) => {
          ...
          async.eachLimit(orgEntities, 1, (orgEntity, next) => {
            ...
            collectionsClient[collectionMethodName](token, localOptions, localCacheOptions, (collectionsError, inneinnerEntitiesrEntity) => {
  ...
  function wrapToFlatten(method) {
    return function wrappedMethod(orgsAndTokens, options, cacheOptions, callback) {
      return method(orgsAndTokens, options, cacheOptions, core.createCallbackWithFlattenedData(callback));
    }
  }
  ...
  return {
    orgMembers: function getAllMembers(orgsAndTokens, options, cacheOptions, callback) {
      ...
      return getCrossOrganizationMethod(
        ...
        options,
        cacheOptions,
        core.createCallbackWithFlattenedData(callback));
    },
