...
/*eslint no-console: ["error", { allow: ["dir"] }] */
...
function finalizeResult(apiContext, response) {
  if (!response || !response.data) {
    console.warn(`${apiContext.redisKey.root} : no response or no response.data`);
    const tempHalt = true;
  }
...
function storeResult(apiContext, response) {
  console.log(`storing result for ${apiContext.redisKey.metadata}`);
  if (!response.data) {
    console.log('no response data to storeResult');
    const haltTemp = true;
  }
...
function createCallbackWithFlattenedData(callback) {
  if (!callback) {
    throw new Error('Undefined callback when required');
  }
  return function callbackFlatEntityFromData(error, entity) {
    console.log(`callbackFlat`);
    if (!error && entity && !entity.data) {
      error = new Error('No entity.data present in the result');
    }
    if (error) {
      // temp:
      console.log(error);
    ...
    // temp
    const remaining = _.difference(Object.getOwnPropertyNames(entity), ['cost', 'meta', 'data']);
    if (remaining.length > 0) {
      console.warn('remaining keys in the response object:');
...
function createCallbackFromEntityData(callback, information) {
  return function callbackFromEntityData(error, data) {
    console.log(`callbackFromEntityData ${information}`)
    if (!error && (!data || !data.data)) {
      error = new Error('No result data from call');
    }
    if (error) {
      // temp error output
      console.log(error);
    ...
    if (data.cost) {
      projected.cost = data.cost;
    }
    if (data.meta) {
      projected.meta = data.meta;
    }
    // temp
    const remaining = _.difference(Object.getOwnPropertyNames(data), ['cost', 'meta', 'data']);
    if (remaining.length > 0) {
      console.warn('remaining keys in the response object:');
      console.dir(remaining);
    }
    // e-temp
    return callback(null, projected);
  }
}
...
module.exports = {
  ...
  createCallbackFromEntityData: createCallbackFromEntityData,
  createCallbackWithFlattenedData: createCallbackWithFlattenedData,
