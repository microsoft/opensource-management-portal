...
function processGithubMetadataBeforeCall(apiContext, metadata) {
  if (metadata && metadata.av && !semver.gt(metadata.av, '4.2.0')) {
    console.log(`${apiContext.redisKey.metadata} was using ${metadata.av}, which is <= to 4.2.0; schema break`); // >= 5.0.1 is really the new schema
    metadata = undefined;
  } else if (metadata && !metadata.av) {
    console.log('restAPI, no AV stored; claiming no metadata available');
    metadata = undefined;
  }
...
function githubReduceMetadataToCache(apiContext, response) {
  ...
  if (metadata && metadata.etag) {
    ...
    console.log(`+ ${appVersion} adding app version to REST API caching metadata *NEW*`);
    if (metadata.link) {
...
console.dir(reduced);
    ...
    console.warn('no metadata.etag, debug?');
    console.log('is this a new thing or not?');
  }
  return metadata;
}
