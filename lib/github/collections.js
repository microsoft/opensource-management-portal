...
function createIntelligentMethods(libraryContext, githubCall) {
      ...
      if (error) {
        ...
        if (result) {
          ...
          if (Array.isArray(result)) {
            // Before node-github v9.0.0, the data was at the root of the results object
  ...
  function generalizedCollectionWithFilter(name, githubClientMethod, propertiesToKeep, token, options, cacheOptions, callback) {
    return generalizedCollectionMethod(token, name, getCollectionAndFilter(token, options, githubClientMethod, propertiesToKeep), options, cacheOptions, core.createCallbackWithFlattenedData(callback));
  }
