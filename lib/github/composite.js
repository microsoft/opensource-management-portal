...
function usingMetadataIsCacheValid(apiContext, metadata) {
  ...
  if (maxAgeSeconds && updatedIso) {
    ...
    if (!metadata) {
      debug('composite: empty/no metadata ' + apiContext.redisKey.metadata);
    } else {
      debug('composite: no updated ' + apiContext.redisKey.metadata);
    }
  }
...
function usingResponseIsCacheValid(apiContext, response) {
  ...
  if (!response.meta) {
    throw new Error('No metadata was provided alongside the API response.');
  }
