...
function decryptEntity(partitionKey, rowKey, properties, encryptionOptions, callback) {
  ...
  try {
    tableEncryptionKey = JSON.parse(properties[tableEncryptionKeyDetails]);
  } catch (parseError) {
  ...
  resolveKeyEncryptionKeyFromOptions(encryptionOptions, wrappedContentKeyIdentifier, (kvkLocateError, kvk) => {
    ...
    unwrapContentKey(keyWrappingAlgorithm, keyEncryptionKeyValue, wrappedContentKeyEncryptedKey, (unwrapError, contentEncryptionKey) => {
      ...
      let listOfEncryptedProperties = null;
      try {
        listOfEncryptedProperties = JSON.parse(decryptedPropertiesSet.toString('utf8'));
        return callback(null, decryptProperties(properties, new Set(listOfEncryptedProperties), partitionKey, rowKey, contentEncryptionKey, tableEncryptionKey, iv));
      } catch (error) {
