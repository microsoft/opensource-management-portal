...
router.use(asyncHandler(async (req: IOrganizationSettings, res, next) => {
  const { organization } = req;
  const { organizationSettingsProvider } = getProviders(req);
  try {
    const dynamicSettings = await organizationSettingsProvider.getOrganizationSetting(String(organization.id));
...
router.get('/', asyncHandler(async (req: IOrganizationSettings, res, next) => {
...
router.get('/features', asyncHandler(async (req: IOrganizationSettings, res, next) => {
...
router.get('/feature/:flag', asyncHandler(async (req: IOrganizationSettings, res, next) => {
  const { dynamicSettings, organization } = req;
...
router.put('/feature/:flag', asyncHandler(async (req: IOrganizationSettings, res, next) => {
  const { dynamicSettings, organization } = req;
...
router.delete('/feature/:flag', asyncHandler(async (req: IOrganizationSettings, res, next) => {
...
router.get('/properties', asyncHandler(async (req: IOrganizationSettings, res, next) => {
  const { dynamicSettings, organization } = req;
...
router.get('/property/:flag', asyncHandler(async (req: IOrganizationSettings, res, next) => {
  const { dynamicSettings, organization } = req;
...
router.put('/property/:propertyName', asyncHandler(async (req: IOrganizationSettings, res, next) => {
  const { organization, dynamicSettings } = req;
  const { insights, organizationSettingsProvider } = getProviders(req);
  const { properties } = dynamicSettings;
  ...
  try {
    ...
    return next(jsonError(`error setting property "${propertyName}" to "${newValue}": ${error}`, ErrorHelper.GetStatus(error) || 400));
  }
  return res.json({
    property: propertyName,
    value: properties[propertyName] || null,
...
router.delete('/property/:propertyName', asyncHandler(async (req: IOrganizationSettings, res, next) => {
  const { organization, dynamicSettings } = req;
  const { organizationSettingsProvider, insights } = getProviders(req);
  const { properties } = dynamicSettings;
  const propertyName = req.params.propertyName as string;
  ...
  try {
    ...
    return next(jsonError(`error removing property "${propertyName}": ${error}`, ErrorHelper.GetStatus(error) || 400));
  }
  return res.json({
    property: propertyName,
    value: properties[propertyName] || null,
