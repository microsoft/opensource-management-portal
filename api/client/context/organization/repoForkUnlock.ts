...
import { getContextualRepository, getContextualRepositoryPermissions } from '../../../../middleware/github/repoPermissions';
...
router.post('/approve', asyncHandler(async (req: ReposAppRequest, res, next) => {
  ...
  try {
    ...
    return next(jsonError(`Problem while approving the administrative lock: ${error}`, ErrorHelper.GetStatus(error) || 500));
  }
}));
